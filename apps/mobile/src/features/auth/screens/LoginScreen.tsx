...
export function LoginScreen() {
  ...
  async function handleLogin() {
    ...
    setLoading(true)
    try {
      const user = await mobileAuthApi.login({ email: email.trim(), password })
      setUser(user)
      router.replace(user.onboardingComplete ? '/(app)' : '/(onboarding)')
    } catch (err) {
      if (err instanceof ApiError && err.status === 401) {
        setError(AUTH_STRINGS.ERROR_INVALID_CREDENTIALS)
      } else {
    ...
    >
      <View className="flex-1 px-6 justify-center">
        ...
        <View className="flex-row justify-center mt-8">
          <Text className="text-stone-500 text-sm">{AUTH_STRINGS.NO_ACCOUNT} </Text>
          <TouchableOpacity
