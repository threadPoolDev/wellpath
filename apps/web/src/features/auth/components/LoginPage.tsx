...
export function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setUser(user)
      navigate(user.onboardingComplete ? ROUTES.DASHBOARD : ROUTES.ONBOARDING, { replace: true })
    } catch (err) {
      ...
      <div className="w-full max-w-sm space-y-8">
        ...
        {oauthError ? (
          <div className="rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-800">
            We had trouble connecting your account. Please try again or sign in with email.
          </div>
        ) : null}
